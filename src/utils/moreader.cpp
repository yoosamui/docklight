...
int MoReader::LoadFromFile(const char *filename, void **modata)
{
    ...
    // area pointed to by buf. Upon successful completion, 0 shall be returned.
...
inline int MoReader::getTargetIndex(const char *s)
{
    ...
    while (1) {
        unsigned int index = readFromOffset(hash_offset + 4 * hash_cursor);
        ...
        if (labelMatches(s, index)) return index;
...
{
    if (!mo_data) return text;

    int target_index = getTargetIndex(text);
    if (target_index == -1) return text;
