    ...
    namespace easing
    {
        ...
        class elastic
        {
          public:
            inline static float easeInPow(float t, float b, float c, float d, float power)
            {
                ...
                float a = c;
                float s = p / 4;
                float postFix = a * pow(2, power * (t -= 1));  // this is a fix, again, with
        ...
        class linear
        {
          public:
            ...
            inline static float easeIn(float t, float b, float c, float d) { return c * t / d + b; }
        ...
        std::function<float(float, float, float, float)> bind(Function f, Args... parameters)
        {
            return std::bind(f, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                             std::placeholders::_4, parameters...);
        }

        inline float map(float v, float minIn, float maxIn, float minOut, float maxOut,
        ...
        template <typename Function, typename... Args>
        inline float map(float v, float minIn, float maxIn, float minOut, float maxOut,
                         Function easing, Args... parameters)
        {
            return map(v, minIn, maxIn, minOut, maxOut, bind(easing, parameters...));
        }

        inline float map_clamp(float v, float minIn, float maxIn, float minOut, float maxOut,
                               std::function<float(float, float, float, float)> easing)
        {
