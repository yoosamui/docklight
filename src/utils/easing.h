...
namespace easing_util
{
    ...
    {
      public:
        inline static float easeIn_s(float t, float b, float c, float d,
                                     float s)
        {
            float postFix = t /= d;
        ...
        inline static float easeOut_s(float t, float b, float c, float d,
                                      float s)
        {
            t = t / d - 1;
        ...
        inline static float easeInOut_s(float t, float b, float c, float d,
                                        float s)
        {
            s *= (1.525f);
            if ((t /= d / 2) < 1) {
    ...
    {
      public:
        inline static float easeInPow(float t, float b, float c, float d,
                                      float power)
        {
            if (t == 0) return b;
            if ((t /= d) == 1) return b + c;
            ...
            float s = p / 4;
            float postFix =
                a * pow(2, power * (t -= 1));  // this is a fix, again, with
                                               // post-increment operators
            return -(postFix * sin((t * d - s) * (2 * float(pi)) / p)) + b;
        }

        inline static float easeOutPow(float t, float b, float c, float d,
                                       float power)
        {
            if (t == 0) return b;
            if ((t /= d) == 1) return b + c;
            ...
            float a = c;
            float s = p / 4;
            return (a * pow(2, -power * t) * sin((t * d - s) * (2 * pi) / p) +
                    c + b);
        }

        inline static float easeInOutPow(float t, float b, float c, float d,
                                         float power)
        {
            if (t == 0) return b;
            ...
            if (t < 1) {
                float postFix =
                    a * pow(2, power * (t -= 1));  // postIncrement is evil
      ...
      public:
        ...
        inline static float easeInOut(float t, float b, float c, float d)
        {
            ...
            of joy
             ...
             James George:: The fix refered to above actually broke the
            equation, it would land at 50% all the time at the end copying back
            the original equation from online fixed it...
    ...
    std::function<float(float, float, float, float)> bind(Function f,
                                                          Args... parameters)
    {
    ...
    inline float map(float v, float minIn, float maxIn, float minOut,
                     float maxOut,
    ...
    template <typename Function, typename... Args>
    inline float map(float v, float minIn, float maxIn, float minOut,
                     float maxOut, Function easing, Args... parameters)
    {
        return map(v, minIn, maxIn, minOut, maxOut,
                   bind(easing, parameters...));
    }

    inline float map_clamp(
        float v, float minIn, float maxIn, float minOut, float maxOut,
        std::function<float(float, float, float, float)> easing)
    {
    ...
    template <typename Function, typename... Args>
    inline float map_clamp(float v, float minIn, float maxIn, float minOut,
                           float maxOut, Function easing, Args... parameters)
    {
        return map_clamp(v, minIn, maxIn, minOut, maxOut,
                         bind(easing, parameters...));
    }
    ...
    inline function easing(Function f, Type t)
    {
        static std::map<std::pair<Function, Type>, function> index = {
            ...
            {std::make_pair(Function::Exponential, Type::InOut),
             exp::easeInOut},
            ...
            {std::make_pair(Function::Elastic, Type::InOut),
             elastic::easeInOut},
        };
